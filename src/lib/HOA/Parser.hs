...
hoaParser =
  (~~) >> do
    ...
    if P.size header /= 0 && P.size header /= length states
      ...
      else let names = map (\(s, (n, _, _, _)) -> (State s, n)) states
               labels = map (\(s, (_, l, _, _)) -> (State s, fmap (fmap AP) l)) states
               accept = map (\(s, (_, _, a, _)) -> (State s, fmap (S.map AcceptanceSet) a)) states
               edges = map (\(s, (_, _, _, e)) -> (State s, S.map convertEdge e)) states
            in return
                 HOA
                   { size = length states
                   ...
                   , acceptance = convertAccType <$> P.acceptance header
                   , tool = P.tool header
                   , name = P.name header
  ...
  where
    ...
    convertEdge (s, mFml, mAcc) =
      (map State s, fmap (fmap AP) mFml, fmap (S.map AcceptanceSet) mAcc)
