...
printHOALines hoa@HOA {..} =
  let apNamesSorted = map (quote . atomicPropositionName) $ atomicProps hoa
      nameAcceptanceCond =
        printFormula
          (\case
  ...
  -- AP
        ...
        ++
  -- controllable-AP
         [ "controllable-AP: " ++ unwords (map strIndAP $ toList controllableAPs)
         | not (null controllableAPs)
         ]
        ++
  -- properties
  ...
  where
    ...
    printAcceptanceSets aSets = brCurly $ unwords $ map strIndAcc $ toList aSets
...
printAcceptanceName =
  \case
    ...
    GeneralizedRabin a b c -> "generalized-Rabin " ++ show a ++ " " ++ show b ++ " " ++ show c
