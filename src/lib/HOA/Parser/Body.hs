...
bodyParser ::
     Int
  -> M.Map String (Formula Int)
  -> Parser
       [( Int
        , ( Maybe String
          , Maybe (Formula Int)
          , Maybe (Set Int)
          , Set ([Int], Maybe (Formula Int), Maybe (Set Int))))]
            ...
            then do
              ...
              if expectedEdges == actualEdges
                then do
                  let labeledEdges =
                        zipWith
                          (\n (s, _, mA) -> (s, Just $ implicitLabel numAPs n, mA))
                          [0 ..]
                          edges
                  return (state, (name, mLabel, mAccSet, S.fromList labeledEdges))
                else unexpected
                       $ "Implicit labeled edges should be: "
                           ++ show expectedEdges
                           ++ " but were: "
                           ++ show actualEdges
            else do
              let explicitLabels = foldr (\(_, l, _) -> (&&) (isJust l)) True edges
              if not explicitLabels
                then unexpected "Edge Labels inconsitent (some labeled, some not labeled)"
