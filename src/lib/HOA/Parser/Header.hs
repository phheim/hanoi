...
headerParser = do
  ...
  where
    ...
    apParser hoa =
      if atomicPropositions hoa /= -1
        then errDoubleDef "AP"
        else do
          num <- natParser
          aps <- many stringParser
          if num /= length aps
            then unexpected "number of APs did not match actual APs"
            else headerItemParser
                   hoa
                     { atomicPropositions = num
                     , atomicPropositionName = M.fromAscList $ zip [0 ..] aps
                     }
    aliasParser hoa = do
      _ <- char '@'
      id <- identParser
      expr <- labelExprParser (aliases hoa)
      if M.member id $ aliases hoa
        then errDoubleDef $ "alias " ++ id
        else headerItemParser hoa {aliases = M.insert id expr (aliases hoa)}
    acceptParser hoa =
      if acceptanceSets hoa /= -1
        then errDoubleDef "Acceptance"
        else do
          nat <- natParser
          acc <- accCondParser
          headerItemParser hoa {acceptanceSets = nat, acceptance = acc}
    acceptNameParser hoa =
      if isJust $ acceptanceName hoa
        then errDoubleDef "AcceptanceName"
        else do
          accName <- accNameParser
          headerItemParser hoa {acceptanceName = Just accName}
    toolParser hoa =
      if isJust $ tool hoa
        then errDoubleDef "tool"
        else do
          str <- stringParser
    ...
    errDoubleDef str = unexpected $ str ++ " (already defined)"

-----------------------------------------------------------------------------
versionParser :: Parser String
versionParser = keyword "HOA:" >> identParser
-----------------------------------------------------------------------------
